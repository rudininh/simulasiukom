...
<div class="cat-card p-3 mb-4">
    <form method="post" action="{{ route('admin.questions.store') }}" class="row g-2">@csrf
        ...
        <div class="col-md-2"><select class="form-select" name="difficulty">@foreach(['easy'=>'Mudah','medium'=>'Sedang','hard'=>'Sulit','case'=>'Studi kasus'] as $value=>$label)<option value="{{ $value }}">{{ $label }}</option>@endforeach</select></div>
