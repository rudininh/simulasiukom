...
<div class="cat-card p-4">
    <form method="post" action="{{ route('admin.question-generator.generate') }}" class="row g-3">@csrf
        ...
        <div class="col-md-2"><label class="form-label">Kesulitan</label><select class="form-select" name="difficulty"><option value="easy">Mudah</option><option value="medium">Sedang</option><option value="hard">Sulit</option><option value="case">Studi kasus</option><option value="calculation">Hitungan</option></select></div>
        <div class="col-md-2"><label class="form-label">Tipe</label><select class="form-select" name="question_type"><option>Pemahaman pasal</option><option>Definisi konsep</option><option>Prosedur administratif</option><option>Penerapan aturan</option><option>Analisis kasus</option><option>Etika dan pengambilan keputusan</option><option>Hitungan angka kredit</option></select></div>
